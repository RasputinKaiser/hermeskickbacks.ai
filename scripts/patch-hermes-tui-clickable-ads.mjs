...
if (existsSync(testFile)) {
  patchFile(testFile, (src) => {
    ...
    out = replaceOnce(
      out,
      "import { StatusRule } from '../components/appChrome.js'",
      "import { handleKickbacksAdClick, KickbacksTickerAdText, readKickbacksTickerAd, StatusRule } from '../components/appChrome.js'"
    );
...
function tickerTests() {
  return [
    ...
    "  })",
    "",
    "  it('opens only clicks inside the status-row ad phrase columns', () => {",
    "    const open = vi.fn(() => true)",
    "    const ad = {",
    "      text: 'Sundial: Actually trustworthy AI data engineering + analysis',",
    "      url: 'https://kickbacks.ai/click'",
    "    }",
    "    const inside = { localCol: 5, stopImmediatePropagation: vi.fn() }",
    "    const before = { localCol: 1, stopImmediatePropagation: vi.fn() }",
    "    const after = { localCol: 5 + ad.text.length, stopImmediatePropagation: vi.fn() }",
    "",
    "    handleKickbacksAdClick(before, ad, 3, open)",
    "    handleKickbacksAdClick(after, ad, 3, open)",
    "    expect(open).not.toHaveBeenCalled()",
    "",
    "    handleKickbacksAdClick(inside, ad, 3, open)",
    "",
    "    expect(inside.stopImmediatePropagation).toHaveBeenCalledOnce()",
    "    expect(open).toHaveBeenCalledWith('https://kickbacks.ai/click')",
    "  })",
    "",
